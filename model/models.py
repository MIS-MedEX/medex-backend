...
class Autoencoder(nn.Module):
    def __init__(self,  ch=16):
        ...
        self.enc1 = nn.Sequential(
            Conv2d(3, ch, 5, padding=1),
            nn.MaxPool2d(2),
            nn.BatchNorm2d(ch),
            nn.ReLU(),
            )
        self.enc2 = nn.Sequential(Conv2d(ch, ch*2, 5, padding=1),
            nn.MaxPool2d(2),
            nn.BatchNorm2d(ch*2),
            nn.ReLU(),
            )
        self.enc3 = nn.Sequential(
            Conv2d(ch*2, ch*4, 5, padding=1),
            nn.MaxPool2d(2),
            nn.BatchNorm2d(ch*4),
            nn.ReLU(),          
        )
        self.enc4 = nn.Sequential(
            Conv2d(ch*4, ch*8, 5, padding=1),
            nn.MaxPool2d(2),
            nn.BatchNorm2d(ch*8),
            nn.ReLU(),
        ...
        self.dec4 = nn.Sequential(
            nn.ConvTranspose2d(ch*8, ch*4, 2, stride=2, padding=1),
            nn.ReflectionPad2d(1),
            nn.BatchNorm2d(ch*4),
            nn.ReLU(),
            Conv2d(ch*4, ch*4, 2, padding=1),
            nn.BatchNorm2d(ch*4),
            nn.ReLU()
        )
        self.dec3 = nn.Sequential(
            nn.ConvTranspose2d(ch*8, ch*2, 2, stride=2, padding=1),
            nn.ReflectionPad2d(1),
            nn.BatchNorm2d(ch*2),
            nn.ReLU(),
            Conv2d(ch*2, ch*2, 2, padding=1),
            nn.BatchNorm2d(ch*2),
            nn.ReLU()
        )
        self.dec2 = nn.Sequential(
            nn.ConvTranspose2d(ch*4, ch, 2, stride=2, padding=1),
            nn.ReflectionPad2d(1),
            nn.BatchNorm2d(ch),
            nn.ReLU(),
            Conv2d(ch, ch, 2, padding=1),
            nn.BatchNorm2d(ch),
            nn.ReLU()
        )
        self.dec1 = nn.Sequential(
            nn.ConvTranspose2d(ch*2, ch, 2, stride=2, padding=1),
            nn.ReflectionPad2d(1),
            nn.BatchNorm2d(ch),
            nn.ReLU(),
            Conv2d(ch, 1, 2),
        ...
        # nn.init.normal_(self.dec1[0].bias, mean=3, std=1)
    ...
    def forward(self, x):
        ...
        x6 = torch.cat((x2, self.dec3(x5)), dim=1)
        x7 = torch.cat((x1, self.dec2(x6)), dim=1)
        mask =  self.dec1(x7)
        x = mask*x
...
class target_classifier(nn.Module):
    def __init__(self, oc=14):
        ...
        self.classifier.fc = nn.Linear(in_features=2048, out_features=oc, bias=True)
        self.activation = nn.Sigmoid()
...
class target_classifier_densenet(nn.Module):
    def __init__(self, oc=1, in_c=3):
        super(target_classifier_densenet, self).__init__()
        self.oc = oc
        self.base_network = models.densenet121(pretrained=True)
        self.base_network.conv0 = Conv2d(in_c, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.base_network.classifier = nn.Linear(in_features=1024, out_features=oc, bias=True)
        self.activation = nn.Sigmoid()
...
class target_classifier_multiclass(nn.Module):
    def __init__(self, oc=14, in_c=3):
        super(target_classifier_multiclass, self).__init__()
        self.base_network = models.resnet50(pretrained = True)
        self.base_network.conv1 = Conv2d(in_c, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.oc = oc
        output_size = self.oc
...
if __name__ =='__main__':
    ...
    model = target_classifier_densenet()
    output = model(x)
    print(output)
